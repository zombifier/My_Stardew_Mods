...
| `selph.CustomTapperFramework_IS_CROP_PRODUCE <target>` | Whether the target item is defined as a produce of a crop (and thus eligible for placement in the seedmaker, among other things.|
---
...
item query context's custom fields), while the last two are usable anywhere:
...
| `selph.CustomTapperFramework_SAPLING_OF <itemId>`| Get the first fruit tree or bush sapling that will produce the item provided.|
...
| `ExtraDrops` | List of [item queries](https://stardewvalleywiki.com/Modding:Item_queries#Item_spawn_fields) with some extra fields | A list of item queries that when evaluated yields an item to add alongide the main harvest.<br>IMPORTANT NOTES:<br>* If there are multiple items in the list, *every* query that qualifies will be chosen to drop.<br>* Like the main drop override, you can set `CopyColor` to copy color from the main produce.<br>* Unlike the main drop override, quality and stack is respected. You can set the new field `RollQuality` inside this entry to true (and optionally `HarvestMinQuality` + `HarvestMaxQuality`) so these extra drops has its quality rolled via farming skill and applied fertilizer.|
