...
- [Extra Animal Configuration Framework](#extra-animal-configuration-framework)
   ...
   * [Animal data asset](#animal-data-asset)
      + [Change male/female percentage](#change-malefemale-percentage)
      + [Change a produce's harvest method, actual returned item, and other settings](#change-a-produces-harvest-method-actual-returned-item-and-other-settings)
      + [Animals that can make multiple different products in the same day](#animals-that-can-make-multiple-different-products-in-the-same-day)
      + [Animals that can forage in dirt (ala Harvest Moon chickens)](#animals-that-can-forage-in-dirt-ala-harvest-moon-chickens)
      ...
      + [Animals that can give birth to other animal types](#animals-that-can-give-birth-to-other-animal-types)
      + [Animal spawn data reference](#animal-spawn-data-reference)
      + [Animals that can go outside in rain or winter](#animals-that-can-go-outside-in-rain-or-winter)
      + [Multiple possible house types for an animal](#multiple-possible-house-types-for-an-animal)
      + [Animals that can glow in the dark](#animals-that-can-glow-in-the-dark)
      + [Animals that are faster (or slower) than normal](#animals-that-are-faster-or-slower-than-normal)
...
value a model with the fields detailed below (unless otherwise specified).
...
### Change male/female percentage

| Field Name                         | Type             | Description              |
| ---------------------------------- | ---------------- | ------------------------ |
| `MalePercentage`          | `float`            | The percentage of this species that will be male. This only affects Marnie's message after buying the animal as well as the pronouns used in certain interact messages.|
...
in custom animal data:

| Field Name                         | Type             | Description              |
| ---------------------------------- | ---------------- | ------------------------ |
| `ExtraProduceSpawnList`          | `List<ExtraProduceSpawnData>`   | A list of extra produce slots to determine which additional produce aside from the primary produce this animal can make. One slot corresponds to one produce (ie. if this list has 2 entries, the animal can make 2 extra produce).|
...
| ---------------------------------- | ---------------- | ------------------------ |
| `Id`          | `string`| The ID of this entry. Must be unique.|
| `ItemId`          | `string`| The *unqualified* item ID of the produce item (like the animal's vanilla produce). This field can integrate with the parent animal's `AnimalProduceExtensionData` field.|
...
- Step 1 (optional): In base animal data, set `GrassEatAmount` to `0` so the animal
   doesn't eat grass and hay. And/or set `OutsideForager` (see above) to `true` if you want the
   animal to forage in dirt when outside.

- Step 2: If you want every animal inside a certain building to automatically eat a custom feed, and change
   its interior `Trough` tiles to match, set this field in the `CustomFields` dict in its
   `Data/Buildings` entry:

| Field Name                          |  Description              |
| ---------------------------  | ------------------------ |
| `selph.ExtraAnimalConfig.BuildingFeedOverrideId` | If set to a qualified item ID, the building's hay troughs/hoppers will be modified to accept that item instead of hay, and any animals living inside that building that usually eat hay will instead eat that item.<br>Use this field if you want to override a vanilla building, and building animals, to eat a certain feed type and don't want to add compat to every other animal and animal house interior mods.|

- Step 2 (alt): If, instead, your animal shares its home with other hay eating animals, or multiple
  animals that each separate feed, you can override its feed on a per-animal basis by setting this
  field in custom animal data:

| Field Name                         | Type             | Description              |
| ---------------------------------- | ---------------- | ------------------------ |
| `FeedItemId`              | `string`           | The *qualified* item ID of this animal's food item instead of hay. If set, the animal will need to eat this item in addition to grass and hay if `GrassEatAmount` > 0, or in place of grass and hay if `GrassEatAmount` is 0. If this field is not set and `GrassEatAmount` is 0, then the animal doesn't need to eat.|

- Step 3: Next, you need to make sure the custom feed items are dispensed on the trough properly in
  your animal house interior maps.
  - If you're using the building override field, simply use the `Trough` tile as in vanilla, and they will be
    usable with the specified feed item (via yhand placeing, or autofeed) instead of hay.
  - If you're using per-animal feed override, you need to specify custom trough tiles for those
    feed items. Add tiles in the `Back` layer with the `selph.ExtraAnimalConfig.CustomTrough` property set to
    the qualified item ID of the feed item.

- Step 4: To add silos for your custom feed, first set this field in the `CustomFields` dict in its
  `Data/Buildings` entry, replacing `ItemId1` as needed:

| Field Name                          |  Description              |
| ---------------------------  | ------------------------ |
| `selph.ExtraAnimalConfig.SiloCapacity.(O)ItemId1` | The silo capacity of the specified custom feed item as a number (e.g. 100). |

Next, you need to set the building's tile action to the following action:

| Action Name                          |  Description              |
| ---------------------------  | ------------------------ |
| `selph.ExtraAnimalConfig.CustomFeedSilo (O)ItemId1` | Put the currently held feed into storage, or show the current feed capacity. The item ID is **optional**; if not set it will accept all feed item that can be stored in the building (including vanilla hay).|

If you're adding your custom feed to vanilla silos, you *do* need to change its action as well!

- Step 5 (optional): The vanilla hay hopper should automatically work for any custom feed supported
  by the animal house. If you need more control, you can use this tile action on the building
  interior:

| Action Name                          |  Description              |
| ---------------------------  | ------------------------ |
| `selph.ExtraAnimalConfig.CustomFeedHopper (O)ItemId1` | Put into, or take the specified feed out of storage. Usable only inside an animal building.|
...
Alternatively set these fields in a building's `CustomFields` in its `Data/Buildings` entry:
...
Sheep). This field allows for more possible textures with conditions associated.
